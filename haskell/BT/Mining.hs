...
getMiningAddress :: PersistentConns -> BTIO (Maybe B.ByteString)
getMiningAddress conn = get conn "g:" "global" "mining_address"

setMiningAddress :: PersistentConns -> B.ByteString -> BTIO Bool
setMiningAddress conn = setnx conn "g:" "global" "mining_address"

storeMerkleDiff :: PersistentConns -> HashData -> BTIO ()
...
setMerkleDiff :: PersistentConns -> B.ByteString -> B.ByteString -> BTIO Bool
setMerkleDiff conn merkle value = do
    resp <- set conn "m:" merkle "diff" value
    _ <- expire conn "m:" merkle 600
    return resp

getMerkleDiff :: PersistentConns -> B.ByteString -> BTIO (Maybe B.ByteString)
...
getShareUsername :: PersistentConns -> B.ByteString -> BTIO (Maybe B.ByteString)
getShareUsername conn shareid = get conn "s:" shareid "username"

setSharePayout :: PersistentConns -> B.ByteString -> BTC -> BTIO Bool
setSharePayout conn shareid = setbtc conn "s:" shareid "payout"

getSharePayout :: PersistentConns -> B.ByteString -> BTIO BTC 
getSharePayout conn shareid = getbtc conn "s:" shareid "payout"


getMineRecieved :: PersistentConns -> BTIO BTC
getMineRecieved conn = getbtc conn "g:" "global" "mine_recieved"

setMineRecieved :: PersistentConns -> BTC -> BTIO Bool
setMineRecieved conn = setbtc conn "g:" "global" "mine_recieved"

incrementSharePayout :: PersistentConns -> B.ByteString -> BTC -> BTIO BTC
incrementSharePayout conn shareid = incrementbtc conn "s:" shareid "payout"

setSharePercentPaid :: PersistentConns -> B.ByteString -> BTC -> BTIO Bool
...
getGlobalShares :: PersistentConns -> Double -> Double -> BTIO [B.ByteString]
getGlobalShares conn = zrangebyscore conn "us:" "global_"


getNextShareLevel :: PersistentConns -> Double -> BTIO Double
...
makeShare :: PersistentConns -> B.ByteString -> BTIO B.ByteString
...
addShareUserQueue :: PersistentConns -> BC.ByteString -> BC.ByteString -> Double -> BTIO Integer
addShareUserQueue conn username shareid payout = zadd conn "us:" username payout shareid

remShareUserQueue :: PersistentConns -> BC.ByteString -> BC.ByteString -> BTIO Integer
remShareUserQueue conn username shareid = zrem conn "us:" username [shareid]


addShareGlobalQueue :: PersistentConns -> BC.ByteString -> Double -> BTIO Integer
addShareGlobalQueue conn shareid payout = zadd conn "us:" "global" payout shareid

getCurrentMiningShares :: PersistentConns -> BTIO [B.ByteString]
getCurrentMiningShares conn = zrangebyscore conn "us:" "global" 0.0 0.0

removeGlobalMiningShares :: PersistentConns -> [B.ByteString] -> BTIO Integer
...
getCurrentMiningShare :: PersistentConns -> B.ByteString -> BTIO B.ByteString
