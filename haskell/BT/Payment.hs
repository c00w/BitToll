...
set_payment_amount conn paymentid value = set conn "payment_" paymentid value

set_payment_user :: PersistentConns -> B.ByteString -> B.ByteString -> IO (Bool)
set_payment_user conn paymentid value = set conn "payment_user_" paymentid value

set_payment_done :: PersistentConns -> B.ByteString -> IO (Bool)
set_payment_done conn paymentid = set conn "payment_user_" paymentid "1"
